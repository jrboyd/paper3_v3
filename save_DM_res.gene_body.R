...
ext = 1500
k = ensg_gb$strand == "+"
# ensg_gb[k,"end"] = ensg_gb[k,"start"] + ext
ensg_gb[k,"start"] = ensg_gb[k,"start"] + ext
k = !k
# ensg_gb[k,"start"] = ensg_gb[k,"end"] - ext
ensg_gb[k,"end"] = ensg_gb[k,"end"] - ext

ensg_gb_granges = GRanges(ensg_gb$chrm, 
                            IRanges(ensg_gb$start, ensg_gb$end), 
                            # transcript_id = rownames(ensg_gb),
                            gene_id = ensg_gb$gene_id, 
                            gene_name = ensg_gb$gene_name)
names(ensg_gb_granges) = rownames(ensg_gb)
my_ref = ensg_gb_granges

# dir.create("promoters_stringent", showWarnings = F)

min_fdr=.05
min_FC = 1
save_filename = paste0("input/", paste0("DM_lists.ensg.padj", min_fdr, ".fc", min_FC, ".gene_body.save"))
...
names(all_ensg_lists) = sub("diffbind_", "", names(all_ensg_lists))
names(all_ensg_lists) = sub("_drugs_", " ", names(all_ensg_lists))
names(all_ensg_lists) = sub("_compareDrugs.save", "", names(all_ensg_lists))
hidden = sapply(names(all_ensg_lists), function(x){
  print(x)
  xsplit = strsplit(x, " ")[[1]]
  n = length(xsplit)
  direct = xsplit[n]
  ...
  newsplit[n-3] = to
  newsplit[n-1] = from
  newsplit[n] = switch (direct,
                        up = "down",
                        down = "up"
  )
  all_ensg_lists[[paste(newsplit, collapse = " ")]] <<- all_ensg_lists[[x]]
  
}); remove("hidden")
all_ensg_lists = all_ensg_lists[sort(names(all_ensg_lists))]

all_gene_lists = lapply(all_ensg_lists, function(x){
  out = unique(ensg_ref[x,]$gene_name)
  return(out)
})

save(all_gene_lists, all_ensg_lists, all_diffbind_tables, file = save_filename)
...
for(cl in c("MCF10A", "MCF7")){
  ...
  for(m in marks){
    ...
    saveWorkbook(wb, file = sub(".save", ".xlsx", save_filename))
  }
}