...
for(m in marks){
  setwd(wd)
  STEIN_SUBSET = get_res(res = STEIN_ChIPseq, cell = "MCF10A", mark = m)
  ...
  setwd(output_dir)
  sub_dir = paste0("GREAT_comparisons_MCF10A_promoters_from_ctrl/", m)
  ...
  a = compare_drugs("ctrl", "bza", with = F, prom_only = T)
  a = compare_drugs("ctrl", "e2", with = F, prom_only = T)
  a = compare_drugs("ctrl", "e2bza", with = F, prom_only = T)
  a = compare_drugs("ctrl", "gc10", with = F, prom_only = T)
  a = compare_drugs("ctrl", "gc10bza", with = F, prom_only = T)
  
  a = compare_drugs("bza", "ctrl", with = F, prom_only = T)
  a = compare_drugs("e2", "ctrl", with = F, prom_only = T)
  a = compare_drugs("e2bza", "ctrl", with = F, prom_only = T)
  a = compare_drugs("gc10", "ctrl", with = F, prom_only = T)
  a = compare_drugs("gc10bza", "ctrl", with = F, prom_only = T)
  ...
  cnames = sapply(strsplit(names(ctrl_comparisons), "[- ]"), function(x)paste(x[c(-1,-3)], collapse = " "))
  ...
  pdf(paste0(m, "_from_ctrl_promoters_msig_enrichments.pdf"))
