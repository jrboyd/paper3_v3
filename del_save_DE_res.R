...
filter_DE_res = function(f){
  dat = read.table(f)
  is_up = dat$log2FoldChange < log2(1/MIN_FC)
  is_down = dat$log2FoldChange > log2(MIN_FC)
  is_sig = dat$padj < MIN_ADJP
...
setwd(input_dir)
save(DE_res, file = paste0("DE_res.", MIN_FC, "FC.", MIN_ADJP, "adjP.save"))
