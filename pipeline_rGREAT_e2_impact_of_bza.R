...
for(m in marks){
  ...
  setwd(output_dir)
  sub_dir = paste0("GREAT_comparisons_e2_and_gc10/", m)
  dir.create(sub_dir, recursive = T)
  setwd(sub_dir)
  
  try(detach("package:openxlsx", unload = T), silent = T)
  library(xlsx)
  # a = compare_drugs(subject = "e2bza", query = "e2", with = F)
  # a = compare_drugs("e2", "e2bza", with = F)
  # a = compare_drugs("gc10bza", "gc10", with = F)
  # a = compare_drugs("gc10", "gc10bza", with = F)
  a = compare_drugs("e2", "gc10", with = F)
  a = compare_drugs("gc10", "e2", with = F)
  a = compare_drugs("e2bza", "gc10bza", with = F)
  a = compare_drugs("gc10bza", "e2bza", with = F)
  ...
  pdf(paste0(m, "_msig_enrichments.pdf"))
