...
is_k4ac = grepl("H3K4AC", names(STEIN_peaks))
STEIN_k4ac_proms = lapply(STEIN_peaks[is_k4ac], function(peaks){
  # peaks = STEIN_peaks$MCF7_e2_H3K4AC
...
STEIN_k4ac_genes = lapply(STEIN_k4ac_proms, function(x){
  x$gene_name
})

STEIN_tscripts = lapply(STEIN_k4ac_proms, function(x){
  x$transcript_id
})

all_genes = unique(unlist(STEIN_k4ac_genes))
all_mat = matrix(F, nrow = length(all_genes), ncol = length(STEIN_k4ac_genes))
colnames(all_mat) = names(STEIN_k4ac_genes)
rownames(all_mat) = all_genes
for(i in colnames(all_mat)){
  all_mat[STEIN_k4ac_genes[[i]], i] = T
}
...
STEIN_k4ac_tscripts = lapply(STEIN_k4ac_genes, function(x){
  best_ids[x]
})

best_k4ac_tscripts = unique(unlist(STEIN_k4ac_tscripts))
k4ac_ref = prom_ref[best_k4ac_tscripts,]
k4ac_ref[,"name"] = paste(k4ac_ref$transcript_id, k4ac_ref$gene_id, k4ac_ref$gene_name, sep = ":")
k4ac_ref[,"score"] = rep(0, nrow(k4ac_ref))
k4ac_bed = as.matrix(k4ac_ref[,c("chrm", "start", "end", "name", "score", "strand" )])
k4ac_bed = gsub(" ", "", k4ac_bed)
write.table(k4ac_bed, file = "STEIN_k4ac_promoters_4ngsplot.bed", row.names = F, col.names = F, quote = F, sep = "\t")
