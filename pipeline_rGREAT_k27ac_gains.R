...
STEIN_MCF7_H3K27AC = get_res(res = STEIN_ChIPseq, cell = "MCF7", mark = "H3K27AC")

#filter peak sets down to smallest set
MIN_PEAKS = min(sapply(STEIN_MCF7_H3K27AC, length))
STEIN_MCF7_H3K27AC = lapply(STEIN_MCF7_H3K27AC, function(x){
...
# cov = coverage(GRangesList(STEIN_MCF7_H3K27AC))
# cov1 = coverage(STEIN_MCF7_H3K27AC[[1]]) + coverage(STEIN_MCF7_H3K27AC[[2]])
# olaps = findOverlaps(STEIN_MCF7_H3K27AC$MCF7_bza_H3K27AC, STEIN_MCF7_H3K27AC$MCF7_e2bza_H3K27AC)
# STEIN_MCF7_H3K27AC$MCF7_bza_H3K27AC[-queryHits(olaps)][500:520]
# 
# # vp_ctrl_v_e2 = venn_peaks(list(ctrl = STEIN_MCF7_H3K27AC$MCF7_ctrl_H3K27AC, e2 =STEIN_MCF7_H3K27AC$MCF7_e2_H3K27AC))
# is_cov = GRanges(coverage(GRangesList(STEIN_MCF7_H3K27AC[2:3])) > 0)
# ctrl_or_e2 = is_cov[is_cov$score]
# wrap_great(subject38 = ctrl_or_e2, query38 = STEIN_MCF7_H3K27AC$MCF7_e2_H3K27AC, 
#            subject_name = "ctrl-e2", query_name = "e2", with = T)
# 
# is_cov = GRanges(coverage(GRangesList(list(STEIN_MCF7_H3K27AC$MCF7_e2_H3K27AC, STEIN_MCF7_H3K27AC$MCF7_bza_H3K27AC))) > 0)
# e2_or_bza = is_cov[is_cov$score]
# wrap_great(subject38 = e2_or_bza, query38 = STEIN_MCF7_H3K27AC$MCF7_bza_H3K27AC, 
#            subject_name = "e2-bza", query_name = "bza", with = F)



compare_drugs = function(subject, query, with = T, esr1_only = F, enh_only = F){
  tmp = get_res(STEIN_MCF7_H3K27AC, from = subject)
  if(length(tmp) != 1) stop("bad subject match")
  subject_gr = tmp[[1]]
  tmp = get_res(STEIN_MCF7_H3K27AC, from = query)
...
setwd(output_dir)
sub_dir = "GREAT_comparisons/to_ctrl_truncated"
dir.create(sub_dir)
setwd(sub_dir)
...
hmap_col = rgb(colorRamp(c("white", "beige", "red", "magenta"))(0:100/100)/255)

pdf("k27ac_msig_enrichments.pdf")
heatmap.2(mat[kept,], margins = c(8,15), trace = "n", col = hmap_col, cexRow = .35)
dev.off()
